// app/product-orderStatus/components/list/product-orderStatus-list.tsx
...
import { createOrderStatusColumns } from './product-orderStatus-columns';
import { useOrderStatusContext } from './product-orderStatus-context';
import { OrderStatus } from '@/api/product-orderStatus.types';
import OrderStatusHeaderList from './product-orderStatus-header-list';
...
export function OrderStatusList<TData extends OrderStatus, TValue>({
  columns,
  sortColumns,
  defaultSortField,
  ...
  const { mutate: deactiveOrderStatus, isPending: isDeactivating } = useDeactiveOrderStatus();

  const handlers = useMemo(
    () => ({
      onView: (orderStatus: OrderStatus) => {
        setViewingOrderStatus(orderStatus);
      },
      onEdit: (orderStatus: OrderStatus) => {
        setEditingOrderStatus(orderStatus);
      },
      onDelete: (orderStatus: OrderStatus) => {
        setDeletingOrderStatus(orderStatus);
      },
      onActive: (orderStatusId: string) => {
        activeOrderStatus(orderStatusId, {
          onSuccess: () => {
      ...
      onDeactive: (orderStatusId: string) => {
        deactiveOrderStatus(orderStatusId, {
          onSuccess: () => {
  ...
  const cols = useMemo(() => {
    return createOrderStatusColumns(handlers) as ColumnDef<TData, TValue>[];
  }, [handlers]);
  ...
  const { data, isLoading } = useGetOrderStatuss({
    page,
    q,
    limit,
    sortField,
                ...
                {!isLoading &&
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      onClick={() => handlers.onView(row.original as unknown as OrderStatus)}
                    >
