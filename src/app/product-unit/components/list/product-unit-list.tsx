// app/product-unit/components/list/product-unit-list.tsx

import React, { useMemo, useState } from 'react';
import { closestCenter, DndContext } from '@dnd-kit/core';
import { restrictToVerticalAxis } from '@dnd-kit/modifiers';
import { useGetUnits, useActiveUnit, useDeactiveUnit } from '@/app/product-unit/api';
...
import { createUnitColumns } from './product-unit-columns';
import { useUnitContext } from './product-unit-context';
import { Unit } from '@/api/product-unit.types';
import UnitHeaderList from './product-unit-header-list';
...
export function UnitList<TData extends Unit, TValue>({
  columns,
  sortColumns,
  defaultSortField,
  ...
  const { setViewingUnit, setEditingUnit, setDeletingUnit } = useUnitContext();

  // Add the mutation hooks
  const { mutate: activeUnit, isPending: isActivating } = useActiveUnit();
  const { mutate: deactiveUnit, isPending: isDeactivating } = useDeactiveUnit();
  ...
  const handlers = useMemo(
    ...
    [setViewingUnit, setEditingUnit, setDeletingUnit, activeUnit, deactiveUnit]
  );

  const cols = useMemo(() => {
    return createUnitColumns(handlers) as ColumnDef<TData, TValue>[];
  }, [handlers]);
                ...
                {!isLoading &&
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      onClick={() => handlers.onView(row.original as unknown as Unit)}
                    >
