...
import { useGetVehicles, useActiveVehicle, useDeactiveVehicle } from '@/app/vehicle/api';
...
import VehicleHeaderList from './vehicle-header-list';
...
export function VehicleList<TData extends ServiceOrder, TValue>({
  columns,
  sortColumns,
  defaultSortField,
  ...
  const { setViewingVehicle, setEditingVehicle, setDeletingVehicle } = useVehicleContext();

  // Add the mutation hooks
  const { mutate: activeVehicle, isPending: isActivating } = useActiveVehicle();
  const { mutate: deactiveVehicle, isPending: isDeactivating } = useDeactiveVehicle();
  ...
  const handlers = useMemo(
    () => ({
      ...
      onActive: (vehicleId: string) => {
        activeVehicle(vehicleId, {
          onSuccess: () => {
            toast.success('Veículo ativado com sucesso!');
          },
          onError: (error: any) => {
            const errorMessage = error.response?.data?.message || 'Erro ao ativar veículo';
            toast.error(errorMessage);
          },
        });
      },
      onDeactive: (vehicleId: string) => {
        deactiveVehicle(vehicleId, {
          onSuccess: () => {
            toast.success('Veículo desativado com sucesso!');
          },
          onError: (error: any) => {
            const errorMessage = error.response?.data?.message || 'Erro ao desativar veículo';
            toast.error(errorMessage);
          },
        });
      },
    }),
    [setViewingVehicle, setEditingVehicle, setDeletingVehicle, activeVehicle, deactiveVehicle]
  );

  const cols = useMemo(() => {
    return createVehicleColumns(handlers) as ColumnDef<TData, TValue>[];
  }, [handlers]);
              ...
              {!isLoading &&
                ...
                table.getRowModel().rows.map((row) => (
                  <TableRow
                    ...
                    onClick={() => handlers.onView(row.original as unknown as Vehicle)}
                  >
