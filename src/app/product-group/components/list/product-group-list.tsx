...
import { createProductGroupColumns } from './product-group-columns';
import { useProductGroupContext } from './product-group-context';
import { ProductGroup } from '@/api/product-group.types';
import ProductGroupHeaderList from './product-group-header-list';
...
export function ProductGroupList<TData extends ProductGroup, TValue>({
  columns,
  sortColumns,
  defaultSortField,
  ...
  const { mutate: deactiveProductGroup, isPending: isDeactivating } = useDeactiveProductGroup();

  const handlers = useMemo(
    () => ({
      onView: (productGroup: ProductGroup) => {
        setViewingProductGroup(productGroup);
      },
      onEdit: (productGroup: ProductGroup) => {
        setEditingProductGroup(productGroup);
      },
      onDelete: (productGroup: ProductGroup) => {
        setDeletingProductGroup(productGroup);
      },
      onActive: (productGroupId: string) => {
        activeProductGroup(productGroupId, {
          onSuccess: () => {
            toast.success('Grupo de produto ativado com sucesso!');
          },
          onError: (error: any) => {
            const errorMessage = error.response?.data?.message || 'Erro ao ativar grupo de produto';
            toast.error(errorMessage);
          },
        });
      },
      onDeactive: (productGroupId: string) => {
        deactiveProductGroup(productGroupId, {
          onSuccess: () => {
  ...
  const cols = useMemo(() => {
    return createProductGroupColumns(handlers) as ColumnDef<TData, TValue>[];
  }, [handlers]);
                ...
                {!isLoading &&
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      onClick={() => handlers.onView(row.original as unknown as ProductGroup)}
                    >
