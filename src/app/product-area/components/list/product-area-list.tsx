...
import { useGetAreas, useActiveArea, useDeactiveArea } from '@/app/product-area/api';
...
import { createAreaColumns } from './product-area-columns';
import { useAreaContext } from './product-area-context';
import { Area } from '@/api/area.types';
import AreaHeaderList from './product-area-header-list';
...
export function AreaList<TData extends Area, TValue>({
  columns,
  sortColumns,
  defaultSortField,
  ...
  const { setViewingArea, setEditingArea, setDeletingArea } = useAreaContext();

  // Add the mutation hooks
  const { mutate: activeArea, isPending: isActivating } = useActiveArea();
  const { mutate: deactiveArea, isPending: isDeactivating } = useDeactiveArea();
  ...
  const handlers = useMemo(
    () => ({
      ...
      onActive: (areaId: string) => {
        activeArea(areaId, {
          onSuccess: () => {
            toast.success('Área ativada com sucesso!');
          },
          onError: (error: any) => {
            const errorMessage = error.response?.data?.message || 'Erro ao ativar área';
            toast.error(errorMessage);
          },
        });
      },
      onDeactive: (areaId: string) => {
        deactiveArea(areaId, {
          onSuccess: () => {
            toast.success('Área desativada com sucesso!');
          },
          onError: (error: any) => {
            const errorMessage = error.response?.data?.message || 'Erro ao desativar área';
            toast.error(errorMessage);
          },
        });
      },
    }),
    [setViewingArea, setEditingArea, setDeletingArea, activeArea, deactiveArea]
  );

  const cols = useMemo(() => {
    return createAreaColumns(handlers) as ColumnDef<TData, TValue>[];
  }, [handlers]);
                ...
                {!isLoading &&
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      onClick={() => handlers.onView(row.original as unknown as Area)}
                    >
